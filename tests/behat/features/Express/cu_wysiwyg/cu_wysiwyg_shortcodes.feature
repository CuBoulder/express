...
I should be able to use the full functionality of the WYSIWYG editor
  ...
  @integration_test
  Scenario: All Express core shortcodes should be enabled on install.
    Given I run the "print_r(shortcode_list_all_enabled('wysiwyg'))" drush eval command at "behat"
    Then the drush output should contain "Buttons designed for donation purposes"
    And the drush output should contain "Creates anchor links to content on the page"
    And the drush output should contain "Callout your content"
    And the drush output should contain "Puts content in a box"
    And the drush output should contain "Make links into buttons"
    And the drush output should contain "Group Buttons"
    And the drush output should contain "Callout content from the main content"
    And the drush output should contain "Clear floated elements"
    And the drush output should contain "Remove the bottom margin from the preceding element"
    And the drush output should contain "Divide content into columns"
    And the drush output should contain "Divide list content into columns"
    And the drush output should contain "Have a number count up to a set value when it comes into view."
    And the drush output should contain "Hide content in expandable/collapsable areas"
    And the drush output should contain "Insert a Google Calendar"
    And the drush output should contain "Include icons in content"
    And the drush output should contain "Add a caption to an image"
    And the drush output should contain "Hide content in an accessible way"
    And the drush output should contain "Insert a Campus or Google Map"
    And the drush output should contain "Display images in a masonry layout"
    And the drush output should contain "Print page button"
    And the drush output should contain "Offset, or callout content"
    And the drush output should contain "Embed content from soundcloud.com"
    And the drush output should contain "Create a link and quote for readers to share on Twitter."
